...
class CustomerPostJDKLockAPISimulation extends Simulation with PerformanceTesterEnvironment {
  ...
  val customerPostJDKLock = exec(http("customerPostJDKLock")
    .post("/customers/jdklock")
    .headers(headers)
    .body(StringBody("""{
                           "id": "${id}",
                           "firstName": "${firstName}",
  ...
  val multiScn = scenario("Kotlin Starter with JDK Lock")
    .repeat(requestsPerUser){ feed(feederRandom).exec(customerPostJDKLock) }
