...
class CustomerPostCacheLockAPISimulation extends Simulation with PerformanceTesterEnvironment {
  ...
  val feederRandom = Iterator.continually {
    Map(
      "id" -> s"${Random.alphanumeric.take(1).mkString}",
  ...
  val customerPostCacheLock = exec(http("customerPostCacheLock")
    .post("/customers/cachelock")
    .headers(headers)
    .body(StringBody("""{
                           "id": "${id}",
                           "firstName": "${firstName}",
  ...
  val multiScn = scenario("Kotlin Starter with Springboot CacheManager Local Lock")
    .repeat(requestsPerUser){ feed(feederRandom).exec(customerPostCacheLock) }
